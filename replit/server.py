...
def send_friend_request(payload: FriendRequestPayload):
    db = _read_db()
    from_user = next((u for u in db["users"] if u["id"] == payload.from_id), None)
    to_user = next((u for u in db["users"] if u["username"].lower() == payload.to_username.lower()), None)
...
def notifications(user_id: int):
    ...
    if not u:
        raise HTTPException(status_code=404, detail="User not found")
    incoming = [
        {"id": fr_id, "username": next((x["username"] for x in db["users"] if x["id"] == fr_id), "")}
...
def list_chats(user_id: int):
    ...
    for uid in peers:
        ...
        chats.append({"id": uid, "username": peer.get("username", ""), "online": online})
...
def get_messages(user_id: int, peer_id: int):
    ...
    msgs = [
        ...
        if (m["from_id"] == user_id and m["to_id"] == peer_id)
        or (m["from_id"] == peer_id and m["to_id"] == user_id)
    ]
...
@app.get("/admin/users")
def admin_users(x_admin_token: str | None = Header(default=None, alias="X-Admin-Token")):
...
def admin_requests(x_admin_token: str | None = Header(default=None, alias="X-Admin-Token")):
    _require_admin(x_admin_token)
    db = _read_db()
    results: list[dict] = []
    ...
    for u in db.get("users", []):
        for from_id in u.get("incoming_requests", []):
            results.append({
                "to_id": u["id"],
                "to_username": u.get("username"),
                "from_id": from_id,
                "from_username": id_to_user.get(from_id, {}).get("username", ""),
            })
...
def admin_reset_password(payload: AdminResetPayload, x_admin_token: str | None = Header(default=None, alias="X-Admin-Token")):
    _require_admin(x_admin_token)
    if not payload.new_password:
        raise HTTPException(status_code=400, detail="Password required")
    db = _read_db()
    user = next((u for u in db.get("users", []) if u.get("id") == payload.user_id), None)
    if not user:
        raise HTTPException(status_code=404, detail="User not found")
...
def admin_delete_user(user_id: int, x_admin_token: str | None = Header(default=None, alias="X-Admin-Token")):
    _require_admin(x_admin_token)
    db = _read_db()
    users = db.get("users", [])
    ...
    db["messages"] = [m for m in db.get("messages", []) if m.get("from_id") != user_id and m.get("to_id") != user_id]
    _write_db(db)
...
def main():
    ...
    srv = UvicornThread(host="127.0.0.1", port=8000)
    srv.start()
